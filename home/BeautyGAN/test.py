...
parser.add_argument('--type_num', type=int, default=1, help='type_num')
...
makeups = glob.glob(os.path.join('imgs', 'makeup', '*.*'))[::-1]
...
# for i in range(3):
makeup = cv2.resize(imread(makeups[args.type_num]), (img_size, img_size))
Y_img = np.expand_dims(preprocess(makeup), 0)
Xs_ = sess.run(Xs, feed_dict={X: X_img, Y: Y_img})
Xs_ = deprocess(Xs_)
# result[:img_size,:img_size] = makeup / 255.
